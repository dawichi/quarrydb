...
| 2026-06-12 | Added `docs/docker-sample-databases-plan.md` to scope a "Try with Docker" sample-database flow before implementation: shared `DockerSampleService` (Docker detection, container lifecycle, port allocation) plus per-provider sample configs, MySQL first; requires adding `tauri-plugin-shell` scoped to a `docker`-only command allowlist |
